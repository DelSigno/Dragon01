...
public class ExpandingDragon implements DrawCollection {
	...
	public ExpandingDragon(Context context, TextureShader2DProgram program){
		...
		this.vertexArray = TextureMapVertexCreator.getHomeSquare(12,new Square(BODY_NODE_SIZE).getData());
		this.vertexArray.bindToBuffer();
		
		this.textureVertexArray = TextureMapVertexCreator.createTextureVertexArray(12,1);
		this.textureVertexArray.bindToBuffer();
		
		this.indiceArray = TextureMapVertexCreator.getHomeSqareIndices((short)12);
	...
	public class BodyNode implements Drawable {
		...
		public void draw() {
			...
			textureVertexArray.attribPointer(program.getTextureCoordinatesAttributeLocation(), TRIS_COUNT, TEXTURE_COMPONENT_COUNT * BYTES_PER_FLOAT, mTimer.getNewOffset() * (POSITION_COMPONENT_COUNT * BYTES_PER_FLOAT));
