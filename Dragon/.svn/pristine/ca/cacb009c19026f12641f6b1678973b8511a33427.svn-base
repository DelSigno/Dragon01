...
public class OrthoCamera {
	...
	private static final String TAG = "OrthoCamera";
	//MATRICIES
	...
	private float[] scaleMatrix = new float[16]; //Used for translations...again
	private float[] rotationMatrix = new float[16]; //Used for translations...again
	...
	public OrthoCamera(int width, int height){
		...
		float screenWidth = 2;
		float screenHeight = 2 * (aspectRatio);
	...
	public float[] srtToObject2D(float sX, float sY, float sZ, float angle,float tX, float tY, float tZ){
		...
		rotateM(rotationMatrix,0,angle, 0, 0, 1);
		setIdentityM(translationMatrix, 0);
		translateM(translationMatrix, 0, tX, tY, tZ);
		setIdentityM(modelMatrix, 0);
